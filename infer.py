...
if __name__ == '__main__':
    ...
    col_name = old_data_pd.columns.values.tolist()
    mean_dict = {name:val for name, val in zip(col_name, mean)}
    old_data = torch.tensor(data_preprocessing(old_data_pd, fill_value=mean_dict).values[:, 1:].astype(np.float32)).unsqueeze(0)
